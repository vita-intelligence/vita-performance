...
export default function ItemStep({ token, selected, onSelect, onNext, onBack }: ItemStepProps) {
    ...
    return (
        <div className="flex flex-col h-full px-6 py-8 gap-6">
            ...
            {/* Selected */}
            {selected && (
                <div className="flex items-center justify-between px-6 py-4 border border-success bg-success/10">
                    <span className="text-lg font-black text-success uppercase">{selected.name}</span>
                    <button
            ...
            <input
                ...
                className="w-full px-4 py-4 text-lg bg-surface border border-border focus:border-text outline-none text-text placeholder:text-muted"
            />
                ...
                <div className="flex flex-col gap-2 overflow-y-auto flex-1">
                    {results.map((item) => (
                        <Button
                            ...
                            variant="bordered"
                            className="w-full justify-start px-6 h-14 text-base font-black uppercase tracking-wide rounded-none border-border text-text hover:border-text hover:bg-surface"
                        >
                            {item.name}
                        </Button>
                    ))}
