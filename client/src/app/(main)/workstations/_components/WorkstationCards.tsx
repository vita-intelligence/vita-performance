...
export default function WorkstationCards({ workstations, onEdit }: WorkstationCardsProps) {
    ...
    const [viewingSOPId, setViewingSOPId] = useState<number | null>(null);
    const [viewingSOPName, setViewingSOPName] = useState<string>("");
    const { sop } = useSOP(viewingSOPId);
    ...
    return (
        <>
                ...
                {workstations.map((workstation) => (
                    <div key={workstation.id} className="border border-border bg-background p-4 flex flex-col gap-4">
                        ...
                        <div className="flex gap-3 border-t border-border pt-4">
                            <button
                                onClick={() => { setViewingSOPId(workstation.id); setViewingSOPName(workstation.name); }}
            ...
            {viewingSOPId && sop && (
                <SOPViewer
                    sop={sop}
                    workstationName={viewingSOPName}
